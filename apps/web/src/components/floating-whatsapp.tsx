...
export function FloatingWhatsApp({ number }: { number: string }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="fab fab--whatsapp group fixed bottom-[calc(2rem+env(safe-area-inset-bottom,0px))] left-[max(0.75rem,env(safe-area-inset-left))] z-[55] touch-manipulation rounded-full bg-[#25D366] font-label font-medium uppercase tracking-wide text-white shadow-soft sm:bottom-8 sm:left-8"
