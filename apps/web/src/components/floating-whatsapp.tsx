...
export function FloatingWhatsApp({ number }: { number: string }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="fixed bottom-[max(1rem,env(safe-area-inset-bottom))] left-[max(0.75rem,env(safe-area-inset-left))] z-[55] flex max-w-[calc(100vw-5rem)] touch-manipulation items-center gap-2 rounded-full bg-[#25D366] px-3 py-3 text-white shadow-soft transition-opacity hover:opacity-95 active:opacity-90 sm:bottom-8 sm:left-8 sm:gap-3 sm:px-4 sm:py-3.5"
      aria-label="Hablemos por WhatsApp"
    >
      <svg viewBox="0 0 32 32" className="size-8 shrink-0 sm:size-9" aria-hidden>
        <path
          fill="currentColor"
      ...
      <span className="min-w-0 font-label text-[10px] font-medium uppercase leading-tight tracking-wide sm:text-[11px] sm:tracking-wider">
